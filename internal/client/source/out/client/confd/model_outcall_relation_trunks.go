/*
accent-confd

Confd exposes an API for managing core resources on a Accent server such as users, extensions, devices, voicemails, queues, etc. Resources can be associated together to provide additional functionality. For example: By associating a voicemail with a user, calls will automatically fallback on to the voicemail when the user cannot answer.  Implementation notes ====================  Errors ------  Responses containing errors will have a status code in the 400 or 500 class. A list of error messages will be returned in the body of the response as a JSON-encoded array:  ~~~ [     \"Input error - User not found\",     \"Resource error - User not associated to a line\" ] ~~~   Updating resources via PUT --------------------------  When updating a resource, all fields become optional. In other words, only values that have been changed need to be sent to the server. Please note that this behavior may change in future versions of the API.

API version: 1.1
Contact: help@accentvoice.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package confd

import (
	"encoding/json"
)

// checks if the OutcallRelationTrunks type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &OutcallRelationTrunks{}

// OutcallRelationTrunks struct for OutcallRelationTrunks
type OutcallRelationTrunks struct {
	Trunks []OutcallRelationTrunk `json:"trunks,omitempty"`
}

// NewOutcallRelationTrunks instantiates a new OutcallRelationTrunks object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewOutcallRelationTrunks() *OutcallRelationTrunks {
	this := OutcallRelationTrunks{}
	return &this
}

// NewOutcallRelationTrunksWithDefaults instantiates a new OutcallRelationTrunks object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewOutcallRelationTrunksWithDefaults() *OutcallRelationTrunks {
	this := OutcallRelationTrunks{}
	return &this
}

// GetTrunks returns the Trunks field value if set, zero value otherwise.
func (o *OutcallRelationTrunks) GetTrunks() []OutcallRelationTrunk {
	if o == nil || IsNil(o.Trunks) {
		var ret []OutcallRelationTrunk
		return ret
	}
	return o.Trunks
}

// GetTrunksOk returns a tuple with the Trunks field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OutcallRelationTrunks) GetTrunksOk() ([]OutcallRelationTrunk, bool) {
	if o == nil || IsNil(o.Trunks) {
		return nil, false
	}
	return o.Trunks, true
}

// HasTrunks returns a boolean if a field has been set.
func (o *OutcallRelationTrunks) HasTrunks() bool {
	if o != nil && !IsNil(o.Trunks) {
		return true
	}

	return false
}

// SetTrunks gets a reference to the given []OutcallRelationTrunk and assigns it to the Trunks field.
func (o *OutcallRelationTrunks) SetTrunks(v []OutcallRelationTrunk) {
	o.Trunks = v
}

func (o OutcallRelationTrunks) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o OutcallRelationTrunks) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Trunks) {
		toSerialize["trunks"] = o.Trunks
	}
	return toSerialize, nil
}

type NullableOutcallRelationTrunks struct {
	value *OutcallRelationTrunks
	isSet bool
}

func (v NullableOutcallRelationTrunks) Get() *OutcallRelationTrunks {
	return v.value
}

func (v *NullableOutcallRelationTrunks) Set(val *OutcallRelationTrunks) {
	v.value = val
	v.isSet = true
}

func (v NullableOutcallRelationTrunks) IsSet() bool {
	return v.isSet
}

func (v *NullableOutcallRelationTrunks) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOutcallRelationTrunks(val *OutcallRelationTrunks) *NullableOutcallRelationTrunks {
	return &NullableOutcallRelationTrunks{value: val, isSet: true}
}

func (v NullableOutcallRelationTrunks) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOutcallRelationTrunks) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
