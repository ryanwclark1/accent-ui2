/*
accent-confd

Confd exposes an API for managing core resources on a Accent server such as users, extensions, devices, voicemails, queues, etc. Resources can be associated together to provide additional functionality. For example: By associating a voicemail with a user, calls will automatically fallback on to the voicemail when the user cannot answer.  Implementation notes ====================  Errors ------  Responses containing errors will have a status code in the 400 or 500 class. A list of error messages will be returned in the body of the response as a JSON-encoded array:  ~~~ [     \"Input error - User not found\",     \"Resource error - User not associated to a line\" ] ~~~   Updating resources via PUT --------------------------  When updating a resource, all fields become optional. In other words, only values that have been changed need to be sent to the server. Please note that this behavior may change in future versions of the API.

API version: 1.1
Contact: help@accentvoice.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package confd

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the DestinationUser type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DestinationUser{}

// DestinationUser struct for DestinationUser
type DestinationUser struct {
	// The UUID of the music on hold to use instead of a ringback tone.
	MohUuid  *string  `json:"moh_uuid,omitempty"`
	RingTime *float32 `json:"ring_time,omitempty"`
	// MUST be 'user'
	Type string `json:"type"`
	// The id of the user
	UserId int32 `json:"user_id"`
}

type _DestinationUser DestinationUser

// NewDestinationUser instantiates a new DestinationUser object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDestinationUser(type_ string, userId int32) *DestinationUser {
	this := DestinationUser{}
	this.Type = type_
	this.UserId = userId
	return &this
}

// NewDestinationUserWithDefaults instantiates a new DestinationUser object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDestinationUserWithDefaults() *DestinationUser {
	this := DestinationUser{}
	return &this
}

// GetMohUuid returns the MohUuid field value if set, zero value otherwise.
func (o *DestinationUser) GetMohUuid() string {
	if o == nil || IsNil(o.MohUuid) {
		var ret string
		return ret
	}
	return *o.MohUuid
}

// GetMohUuidOk returns a tuple with the MohUuid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DestinationUser) GetMohUuidOk() (*string, bool) {
	if o == nil || IsNil(o.MohUuid) {
		return nil, false
	}
	return o.MohUuid, true
}

// HasMohUuid returns a boolean if a field has been set.
func (o *DestinationUser) HasMohUuid() bool {
	if o != nil && !IsNil(o.MohUuid) {
		return true
	}

	return false
}

// SetMohUuid gets a reference to the given string and assigns it to the MohUuid field.
func (o *DestinationUser) SetMohUuid(v string) {
	o.MohUuid = &v
}

// GetRingTime returns the RingTime field value if set, zero value otherwise.
func (o *DestinationUser) GetRingTime() float32 {
	if o == nil || IsNil(o.RingTime) {
		var ret float32
		return ret
	}
	return *o.RingTime
}

// GetRingTimeOk returns a tuple with the RingTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DestinationUser) GetRingTimeOk() (*float32, bool) {
	if o == nil || IsNil(o.RingTime) {
		return nil, false
	}
	return o.RingTime, true
}

// HasRingTime returns a boolean if a field has been set.
func (o *DestinationUser) HasRingTime() bool {
	if o != nil && !IsNil(o.RingTime) {
		return true
	}

	return false
}

// SetRingTime gets a reference to the given float32 and assigns it to the RingTime field.
func (o *DestinationUser) SetRingTime(v float32) {
	o.RingTime = &v
}

// GetType returns the Type field value
func (o *DestinationUser) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *DestinationUser) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *DestinationUser) SetType(v string) {
	o.Type = v
}

// GetUserId returns the UserId field value
func (o *DestinationUser) GetUserId() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.UserId
}

// GetUserIdOk returns a tuple with the UserId field value
// and a boolean to check if the value has been set.
func (o *DestinationUser) GetUserIdOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.UserId, true
}

// SetUserId sets field value
func (o *DestinationUser) SetUserId(v int32) {
	o.UserId = v
}

func (o DestinationUser) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DestinationUser) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.MohUuid) {
		toSerialize["moh_uuid"] = o.MohUuid
	}
	if !IsNil(o.RingTime) {
		toSerialize["ring_time"] = o.RingTime
	}
	toSerialize["type"] = o.Type
	toSerialize["user_id"] = o.UserId
	return toSerialize, nil
}

func (o *DestinationUser) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"type",
		"user_id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varDestinationUser := _DestinationUser{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varDestinationUser)

	if err != nil {
		return err
	}

	*o = DestinationUser(varDestinationUser)

	return err
}

type NullableDestinationUser struct {
	value *DestinationUser
	isSet bool
}

func (v NullableDestinationUser) Get() *DestinationUser {
	return v.value
}

func (v *NullableDestinationUser) Set(val *DestinationUser) {
	v.value = val
	v.isSet = true
}

func (v NullableDestinationUser) IsSet() bool {
	return v.isSet
}

func (v *NullableDestinationUser) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDestinationUser(val *DestinationUser) *NullableDestinationUser {
	return &NullableDestinationUser{value: val, isSet: true}
}

func (v NullableDestinationUser) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDestinationUser) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
