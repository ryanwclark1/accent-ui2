/*
accent-chatd

Control your message and presence from a REST API

API version: 1.0.0
Contact: help@accentvoice.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package chatd

import (
	"encoding/json"
)

// checks if the PresenceList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PresenceList{}

// PresenceList struct for PresenceList
type PresenceList struct {
	// The number of result matching the searched terms
	Filtered *int32     `json:"filtered,omitempty"`
	Items    []Presence `json:"items,omitempty"`
	// The number of results without filter
	Total *int32 `json:"total,omitempty"`
}

// NewPresenceList instantiates a new PresenceList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPresenceList() *PresenceList {
	this := PresenceList{}
	return &this
}

// NewPresenceListWithDefaults instantiates a new PresenceList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPresenceListWithDefaults() *PresenceList {
	this := PresenceList{}
	return &this
}

// GetFiltered returns the Filtered field value if set, zero value otherwise.
func (o *PresenceList) GetFiltered() int32 {
	if o == nil || IsNil(o.Filtered) {
		var ret int32
		return ret
	}
	return *o.Filtered
}

// GetFilteredOk returns a tuple with the Filtered field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PresenceList) GetFilteredOk() (*int32, bool) {
	if o == nil || IsNil(o.Filtered) {
		return nil, false
	}
	return o.Filtered, true
}

// HasFiltered returns a boolean if a field has been set.
func (o *PresenceList) HasFiltered() bool {
	if o != nil && !IsNil(o.Filtered) {
		return true
	}

	return false
}

// SetFiltered gets a reference to the given int32 and assigns it to the Filtered field.
func (o *PresenceList) SetFiltered(v int32) {
	o.Filtered = &v
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *PresenceList) GetItems() []Presence {
	if o == nil || IsNil(o.Items) {
		var ret []Presence
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PresenceList) GetItemsOk() ([]Presence, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *PresenceList) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []Presence and assigns it to the Items field.
func (o *PresenceList) SetItems(v []Presence) {
	o.Items = v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *PresenceList) GetTotal() int32 {
	if o == nil || IsNil(o.Total) {
		var ret int32
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PresenceList) GetTotalOk() (*int32, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *PresenceList) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given int32 and assigns it to the Total field.
func (o *PresenceList) SetTotal(v int32) {
	o.Total = &v
}

func (o PresenceList) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PresenceList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Filtered) {
		toSerialize["filtered"] = o.Filtered
	}
	if !IsNil(o.Items) {
		toSerialize["items"] = o.Items
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}
	return toSerialize, nil
}

type NullablePresenceList struct {
	value *PresenceList
	isSet bool
}

func (v NullablePresenceList) Get() *PresenceList {
	return v.value
}

func (v *NullablePresenceList) Set(val *PresenceList) {
	v.value = val
	v.isSet = true
}

func (v NullablePresenceList) IsSet() bool {
	return v.isSet
}

func (v *NullablePresenceList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePresenceList(val *PresenceList) *NullablePresenceList {
	return &NullablePresenceList{value: val, isSet: true}
}

func (v NullablePresenceList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePresenceList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
