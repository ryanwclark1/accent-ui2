...
for file in $files; do
    ...
    -i /workdir/${filename} \
    -g go-server \
    --additional-properties=packageName=${filename_no_ext_hyphens}server,packageVersion=0.0.1,addResponseHeader=true,enumClassPrefix=false,featureCORS=false,hideGenerationTimestamp=false,onlyInterfaces=true,outputAsLibrary=true,router=chi,serverport=8081,sourceFolder=src \
