/*
accent-confd

Confd exposes an API for managing core resources on a Accent server such as users, extensions, devices, voicemails, queues, etc. Resources can be associated together to provide additional functionality. For example: By associating a voicemail with a user, calls will automatically fallback on to the voicemail when the user cannot answer.  Implementation notes ====================  Errors ------  Responses containing errors will have a status code in the 400 or 500 class. A list of error messages will be returned in the body of the response as a JSON-encoded array:  ~~~ [     \"Input error - User not found\",     \"Resource error - User not associated to a line\" ] ~~~   Updating resources via PUT --------------------------  When updating a resource, all fields become optional. In other words, only values that have been changed need to be sent to the server. Please note that this behavior may change in future versions of the API.

API version: 1.1
Contact: help@accentvoice.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package confd

import (
	"encoding/json"
)

// checks if the QueueMemberAgentRelationBase type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &QueueMemberAgentRelationBase{}

// QueueMemberAgentRelationBase struct for QueueMemberAgentRelationBase
type QueueMemberAgentRelationBase struct {
	// Agent's penalty. A priority used for distributing calls.
	Penalty *int32 `json:"penalty,omitempty"`
	// Priority of agent in the queue. Only used for linear ring strategy
	Priority *int32 `json:"priority,omitempty"`
}

// NewQueueMemberAgentRelationBase instantiates a new QueueMemberAgentRelationBase object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewQueueMemberAgentRelationBase() *QueueMemberAgentRelationBase {
	this := QueueMemberAgentRelationBase{}
	return &this
}

// NewQueueMemberAgentRelationBaseWithDefaults instantiates a new QueueMemberAgentRelationBase object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewQueueMemberAgentRelationBaseWithDefaults() *QueueMemberAgentRelationBase {
	this := QueueMemberAgentRelationBase{}
	return &this
}

// GetPenalty returns the Penalty field value if set, zero value otherwise.
func (o *QueueMemberAgentRelationBase) GetPenalty() int32 {
	if o == nil || IsNil(o.Penalty) {
		var ret int32
		return ret
	}
	return *o.Penalty
}

// GetPenaltyOk returns a tuple with the Penalty field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueueMemberAgentRelationBase) GetPenaltyOk() (*int32, bool) {
	if o == nil || IsNil(o.Penalty) {
		return nil, false
	}
	return o.Penalty, true
}

// HasPenalty returns a boolean if a field has been set.
func (o *QueueMemberAgentRelationBase) HasPenalty() bool {
	if o != nil && !IsNil(o.Penalty) {
		return true
	}

	return false
}

// SetPenalty gets a reference to the given int32 and assigns it to the Penalty field.
func (o *QueueMemberAgentRelationBase) SetPenalty(v int32) {
	o.Penalty = &v
}

// GetPriority returns the Priority field value if set, zero value otherwise.
func (o *QueueMemberAgentRelationBase) GetPriority() int32 {
	if o == nil || IsNil(o.Priority) {
		var ret int32
		return ret
	}
	return *o.Priority
}

// GetPriorityOk returns a tuple with the Priority field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueueMemberAgentRelationBase) GetPriorityOk() (*int32, bool) {
	if o == nil || IsNil(o.Priority) {
		return nil, false
	}
	return o.Priority, true
}

// HasPriority returns a boolean if a field has been set.
func (o *QueueMemberAgentRelationBase) HasPriority() bool {
	if o != nil && !IsNil(o.Priority) {
		return true
	}

	return false
}

// SetPriority gets a reference to the given int32 and assigns it to the Priority field.
func (o *QueueMemberAgentRelationBase) SetPriority(v int32) {
	o.Priority = &v
}

func (o QueueMemberAgentRelationBase) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o QueueMemberAgentRelationBase) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Penalty) {
		toSerialize["penalty"] = o.Penalty
	}
	if !IsNil(o.Priority) {
		toSerialize["priority"] = o.Priority
	}
	return toSerialize, nil
}

type NullableQueueMemberAgentRelationBase struct {
	value *QueueMemberAgentRelationBase
	isSet bool
}

func (v NullableQueueMemberAgentRelationBase) Get() *QueueMemberAgentRelationBase {
	return v.value
}

func (v *NullableQueueMemberAgentRelationBase) Set(val *QueueMemberAgentRelationBase) {
	v.value = val
	v.isSet = true
}

func (v NullableQueueMemberAgentRelationBase) IsSet() bool {
	return v.isSet
}

func (v *NullableQueueMemberAgentRelationBase) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableQueueMemberAgentRelationBase(val *QueueMemberAgentRelationBase) *NullableQueueMemberAgentRelationBase {
	return &NullableQueueMemberAgentRelationBase{value: val, isSet: true}
}

func (v NullableQueueMemberAgentRelationBase) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableQueueMemberAgentRelationBase) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
